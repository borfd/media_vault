...
describe AddToMediaCollection do
  ...
  subject { described_class.new(user: user, url: item_url, type: 'website') }

  it "calls Repository with item_url" do
    expect(MediaRepository).to receive(:add).with({user: user, url: item_url, type: 'website'}).and_return(mock_item)
